...
const { id = "", name = "", type = "text", placeholder = "", label = "" } = defineProps<InputProps>();
defineEmits<Emits>();
...
<template>
	<div class="base-input relative w-full">
		<input :id="id" ref="inputRef" :value="modelValue" :name="name" :type="type" autocomplete="off" class="base-input__element block w-full border-b-[0.05rem] border-solid border-gray transition-colors ease-linear" :class="[{ 'pl-[3.7rem]': !!icon, 'base-input__element--error': isInvalid }]" v-bind="$attrs" :placeholder="placeholder" @input="$emit('update:modelValue', ($event.target as HTMLInputElement).value)" />
